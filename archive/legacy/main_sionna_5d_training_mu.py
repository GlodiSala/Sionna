...
from sionna.phy.channel import OFDMChannel, ApplyOFDMChannel, cir_to_ofdm_channel, subcarrier_frequencies
from sionna.phy.ofdm import ResourceGrid, ResourceGridMapper, LSChannelEstimator, LMMSEEqualizer,OFDMModulator, OFDMDemodulator, RZFPrecoder,RemoveNulledSubcarriers  # ✅ ADDED
from sionna.phy.channel import gen_single_sector_topology as gen_topology
from sionna.phy.channel.tr38901 import AntennaArray, UMi, UMa, RMa  # ✅ CHANGED from CDL
from sionna.phy.fec.ldpc import LDPC5GEncoder, LDPC5GDecoder
from sionna.phy.mapping import Mapper, Demapper, BinarySource, QAMSource
from sionna.phy.utils import ebnodb2no, compute_ber, sim_ber  # ✅ ADDED sim_ber
print(f"✅ Sionna version: {sionna.__version__}")

from transformer_5d_sionna_mu import TransformerPrecoder5D_Fixed
from correct_sum_rate_trainer import *


class MultiUserMIMO_Downlink(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self._num_streams_per_tx = num_ut
        ...
        self._sm = StreamManagement(
            rx_tx_association=np.ones([num_ut, 1]),
            num_streams_per_tx=num_ut
        )
        
        # Antenna Arrays
        self._ut_array = AntennaArray(
            num_rows=1, num_cols=num_ut_ant,
            polarization="single",
            polarization_type="V",
        ...
        if scenario == "umi":
            self._channel_model = UMi(o2i_model="low", **channel_params)
        elif scenario == "uma":
            self._channel_model = UMa(o2i_model="low", **channel_params)
        elif scenario == "rma":
            self._channel_model = RMa(**channel_params)
        ...
        # Precoder Selection
        if precoder_type == "rzf":
            self._precoder = RZFPrecoder(self._rg, self._sm, return_effective_channel=True)
        elif precoder_type == "wmmse":
            self._precoder = WMMSEPrecoder(self._rg, self._sm, num_iter=5)
        elif precoder_type == "transformer_5d":
            self._precoder = TransformerPrecoder5D_Fixed(
                self._rg, self._sm,
                num_tx_antennas=self._num_bs_ant,
                num_rx_antennas=self._num_ut,
                rb_size=self._rb_size
            )
        
        # ✅ Channel components - MUST HAVE
        self._frequencies = subcarrier_frequencies(
            self._rg.fft_size, 
            self._rg.subcarrier_spacing
        )
    ...
    def call(self, batch_size, ebno_db, training=False, return_llr=False):
        """
        ✅ FIXED: Add training and return_llr support
        """
        self.new_topology(batch_size)
        
        no = ebnodb2no(ebno_db, self._num_bits_per_symbol, 
                    self._coderate, self._rg)
        
        # Transmitter
        b = self._binary_source([batch_size, 1, self._num_ut, self._k])
        c = self._encoder(b)
        x = self._mapper(c)
        x_rg = self._rg_mapper(x)
        
        # Channel
        cir = self._channel_model(
            batch_size, 
            self._rg.num_ofdm_symbols,
            1.0 / self._rg.ofdm_symbol_duration
        )
        h_freq = cir_to_ofdm_channel(self._frequencies, *cir, normalize=True)
        
        # Precoding (pass training flag to transformer)
        if self._precoder_type in ["rzf", "wmmse"]:
            x_precoded, h_eff = self._precoder(x_rg, h_freq)
            precoding_matrix = None  # Not used for classical precoders
        elif self._precoder_type == "transformer_5d":
            x_precoded, h_eff, precoding_matrix = self._precoder(
                x_rg, h_freq, training=training
            )
        
        # Channel application
        y = self._apply_channel(x_precoded, h_freq, no)
        ...
        if return_llr:
            # Training mode: return everything
            return b, b_hat, h_freq, h_eff, precoding_matrix, llr, c
        else:
            # Inference mode: just bits
            return b, b_hat

#[Keep the same main() function as before]   

class EndToEndTrainer_Fixed:
    """Memory-Optimized Trainer - works with corrected simulator"""
    ...
    def __init__(self, simulator_5d, learning_rate=5e-4):
        ...
        # ✅ CHECK: Print trainable variables info
        ...
        if simulator_5d._precoder_type == "transformer_5d":
            trainable_vars = simulator_5d._precoder.trainable_variables
            print(f"\n✅ Training Transformer Precoder:")
            ...
            print(f"\n  Trainable variable samples:")
            for i, var in enumerate(trainable_vars[:5]):
                print(f"    {i+1}. {var.name}: shape={var.shape}")
            if len(trainable_vars) > 5:
                print(f"    ... and {len(trainable_vars) - 5} more")
        else:
            print(f"\n⚠️ WARNING: Not training transformer (precoder_type={simulator_5d._precoder_type})")
    ...
    def calculate_sum_rate_corrected(self, precoding_matrix, h_freq, noise_power=None):
        ...
        # ========== CONVERT H_FREQ TO RB FORMAT ==========
        # Clean h_freq: [B, num_rx=4, num_rx_ant=1, num_tx=1, num_tx_ant=8, ofdm=14, fft=72]
        h_clean = tf.squeeze(h_freq, axis=[2, 3])
        # Result: [B, num_rx=4, num_tx_ant=8, ofdm=14, fft=72]
        
        # Average over OFDM symbols
        h_avg = tf.reduce_mean(h_clean, axis=3)  # [B, R, T, fft]
        
        B = tf.shape(h_avg)[0]
        R = tf.shape(h_avg)[1]  # num_rx = 4
        T = tf.shape(h_avg)[2]  # num_tx_ant = 8
        fft_size = tf.shape(h_avg)[3]  # 72
        
        num_rb = fft_size // rb_size  # 72 / 12 = 6
        
        # Transpose to [B, R, fft, T] for easier reshaping
        h_avg = tf.transpose(h_avg, perm=[0, 1, 3, 2])
        
        # Reshape into RB format: [B, R, num_rb, rb_size, T]
        h_rb = tf.reshape(h_avg, [B, R, num_rb, rb_size, T])
        
        # ========== CONVERT PRECODING MATRIX TO RB FORMAT ==========
        # Input: [B, num_tx=1, num_tx_ant=8, num_streams=4, ofdm=14, fft=72]
        
        # Remove num_tx dimension
        W = tf.squeeze(precoding_matrix, axis=1)
        # Result: [B, num_tx_ant=8, num_streams=4, ofdm=14, fft=72]
        
        # Average over OFDM symbols
        W_avg = tf.reduce_mean(W, axis=3)
        # Result: [B, M=8, U=4, fft=72]
        
        # Transpose to [B, U, fft, M]
        W_avg = tf.transpose(W_avg, perm=[0, 2, 3, 1])
        # Result: [B, U=4, fft=72, M=8]
        
        U = tf.shape(W_avg)[1]  # num_streams = 4
        M = tf.shape(W_avg)[3]  # num_tx_ant = 8
        
        # Reshape into RB format: [B, U, num_rb, rb_size, M]
        W_rb = tf.reshape(W_avg, [B, U, num_rb, rb_size, M])
        ...
        # ========== CALCULATE SUM RATE ==========
        # Flatten RB dimension back to frequency for SINR calculation
        F = num_rb * rb_size  # 72
        h_flat = tf.reshape(h_rb, [B, R, F, T])  # [B, R=4, fft=72, T=8]
        W_flat = tf.reshape(W_rb_normalized, [B, U, F, M])  # [B, U=4, fft=72, M=8]
        
        # Calculate effective channel: H * W^H
        # For each receiver r and user u: h_eff[r,u] = sum_f sum_t (h[r,f,t] * conj(W[u,f,t]))
        W_eff = tf.einsum('brft,buft->bru', h_flat, tf.math.conj(W_flat))
        # Result: [B, R=4, U=4] - received signal at each receiver from each user
        
        # Signal power (diagonal elements - each user's own signal)
        signal_power = tf.abs(tf.linalg.diag_part(W_eff)) ** 2  # [B, R]
        
        # Total received power at each receiver
        total_power = tf.reduce_sum(tf.abs(W_eff) ** 2, axis=2)  # [B, R]
        
        # Interference = total power - signal power
        interference = total_power - signal_power
        
        # SINR per user
        SINR = signal_power / (interference + noise_power + 1e-12)  # [B, R]
        
        # Rate per user (Shannon capacity in bits/s/Hz)
        rate_per_user_batch = tf.math.log(1.0 + SINR) / tf.math.log(2.0)  # [B, R]
        rate_per_user = tf.reduce_mean(rate_per_user_batch, axis=0)  # [R]
        
        # Sum rate across all users
        sum_rate_batch = tf.reduce_sum(rate_per_user_batch, axis=1)  # [B]
        avg_sum_rate = tf.reduce_mean(sum_rate_batch)  # scalar
        ...
        metrics = {
            ...
            'rate_per_subcarrier': avg_sum_rate / tf.cast(F, tf.float32),
    ...
    def train_step(self, batch_size, ebno_db, lambda_ber, lambda_power):
        """
        FIXED Training Step - Only train transformer weights
        """
        batch_size_tensor = tf.constant(batch_size, dtype=tf.int32)
        
        with tf.GradientTape() as tape:
            # Forward pass
            b, b_hat, h_freq, h_eff, precoding_matrix, llr, c = self.simulator(
        ...
        # ✅ FIX: Only get transformer trainable variables
        if self.simulator._precoder_type == "transformer_5d":
            trainable_vars = self.simulator._precoder.trainable_variables
        else:
            trainable_vars = self.simulator.trainable_variables
        
        # Check if we have trainable variables
        if len(trainable_vars) == 0:
            tf.print("⚠️ WARNING: No trainable variables found!")
            return total_loss, ber_total, sum_rate, metrics, ber_per_user
        
        # Compute gradients
        gradients = tape.gradient(total_loss, trainable_vars)
        
        # ✅ FIX: Filter out None gradients and clip
        valid_grads_and_vars = []
        for grad, var in zip(gradients, trainable_vars):
            if grad is not None:
                clipped_grad = tf.clip_by_norm(grad, 10.0)
                valid_grads_and_vars.append((clipped_grad, var))
            else:
                tf.print(f"⚠️ None gradient for variable: {var.name}")
        
        # Apply gradients only if we have valid ones
        if len(valid_grads_and_vars) > 0:
            self.optimizer.apply_gradients(valid_grads_and_vars)
        else:
            tf.print("⚠️ WARNING: No valid gradients to apply!")
        ...
        metrics.update({
            ...
            'num_trainable_vars': tf.constant(len(trainable_vars))
        })
    ...
    def train(self, num_epochs=100, batch_size=128, ebno_db_range=(0, 10)):  # ✅ Reduced batch_size
        ...
        for epoch in range(num_epochs):
            ...
            lambda_ber = 0  # Numpy calculation
...
def evaluate_sum_rate_comparison(model_transformer, model_rzf, ebno_db=10.0, num_batches=10):
    """
    Compare sum rates: Transformer vs RZF using identical calculation
    """
    print(f"\n{'='*80}")
    ...
    for i in range(num_batches):
        ...
        # Transformer precoding
        x_prec_tf, h_eff_tf, W_tf = model_transformer._precoder(x_rg, h_freq, training=False)
        
        # Calculate sum rate for transformer
        no = ebnodb2no(ebno_tf, 2, 0.5, model_transformer._rg)
        rate_tf, metrics_tf = calculate_sum_rate_from_effective_channel(
            h_eff_tf, W_tf, no
        )
        
        rates_transformer.append(rate_tf.numpy())
        sinr_transformer.append(metrics_tf['sinr_db'].numpy())
        
        # ========== RZF (same channel) ==========
        # Use SAME channel realization
        x_prec_rzf, h_eff_rzf = model_rzf._precoder(x_rg, h_freq)
        
        # Calculate sum rate for RZF (using same method)
        rate_rzf, metrics_rzf = calculate_sum_rate_from_effective_channel(
            h_eff_rzf, None, no
        )
        
        rates_rzf.append(rate_rzf.numpy())
        sinr_rzf.append(metrics_rzf['sinr_db'].numpy())
        
        if i == 0:
            print(f"\nBatch 1 Details:")
            print(f"  Transformer: Rate={rate_tf:.2f} bps/Hz, SINR={metrics_tf['sinr_db']:.1f} dB")
            print(f"  RZF:         Rate={rate_rzf:.2f} bps/Hz, SINR={metrics_rzf['sinr_db']:.1f} dB")
    ...
    print(f"{'='*80}")
    print(f"Transformer:")
    print(f"  Sum Rate: {np.mean(rates_transformer):.2f} ± {np.std(rates_transformer):.2f} bps/Hz")
    print(f"  SINR:     {np.mean(sinr_transformer):.1f} ± {np.std(sinr_transformer):.1f} dB")
    print(f"\nRZF:")
    print(f"  Sum Rate: {np.mean(rates_rzf):.2f} ± {np.std(rates_rzf):.2f} bps/Hz")
    print(f"  SINR:     {np.mean(sinr_rzf):.1f} ± {np.std(sinr_rzf):.1f} dB")
    
    gain = np.mean(rates_transformer) - np.mean(rates_rzf)
    gain_pct = 100 * gain / np.mean(rates_rzf)
    
    print(f"\nGain:")
    print(f"  Absolute: {gain:+.2f} bps/Hz")
    print(f"  Relative: {gain_pct:+.1f}%")
    print(f"{'='*80}\n")
    
    return {
        'transformer': {'rate': np.mean(rates_transformer), 'sinr': np.mean(sinr_transformer)},
        'rzf': {'rate': np.mean(rates_rzf), 'sinr': np.mean(sinr_rzf)},
        'gain': gain
    }


def calculate_sum_rate_from_effective_channel(h_eff, W, noise_power):
    """
    Calculate sum rate from effective channel (after precoding)
    
    Args:
        h_eff: [B, num_rx, num_rx_ant, num_tx, num_streams, ofdm, num_eff]
        W: precoding matrix (optional, for diagnostics)
        noise_power: scalar
    """
    # Remove singleton dimensions
    h = tf.squeeze(h_eff, axis=[2, 3])  # [B, R, S, ofdm, eff]
    
    # Average over OFDM and frequency
    h_avg = tf.reduce_mean(h, axis=[3, 4])  # [B, R, S]
    
    # Calculate received signal power matrix
    # H_eff is [B, R, S] where R=num_rx (users), S=num_streams
    # For downlink: each receiver r gets signal from all streams s
    
    H_squared = tf.abs(h_avg) ** 2  # [B, R, S]
    
    # Signal power: diagonal elements (intended signal)
    signal_power = tf.linalg.diag_part(H_squared)  # [B, R]
    
    # Total power: sum over all streams
    total_power = tf.reduce_sum(H_squared, axis=2)  # [B, R]
    
    # Interference: total - signal
    interference = total_power - signal_power
    
    # SINR per user
    SINR = signal_power / (interference + noise_power + 1e-12)
    
    # Rate per user (Shannon capacity)
    rate_per_user = tf.math.log(1.0 + SINR) / tf.math.log(2.0)
    
    # Sum rate
    sum_rate = tf.reduce_sum(rate_per_user, axis=1)  # [B]
    avg_sum_rate = tf.reduce_mean(sum_rate)
    
    # Metrics
    avg_sinr_db = 10.0 * tf.math.log(tf.reduce_mean(SINR) + 1e-12) / tf.math.log(10.0)
    
    metrics = {
        'sinr_db': avg_sinr_db,
        'signal_power': tf.reduce_mean(signal_power),
        'interference_power': tf.reduce_mean(interference),
        'rate_per_user': tf.reduce_mean(rate_per_user, axis=0)
    }
...
def main():
    print("\n" + "="*80)
    print("CORRECT TRANSFORMER TRAINING WITH RB GROUPING")
    print("="*80)
    
    # ===== STEP 1: CREATE & BUILD MODEL =====
    print("\n[STEP 1] Creating Transformer Model...")
    
    model_train = MultiUserMIMO_Downlink(
        scenario="umi",
        perfect_csi=True,
    ...
    dummy_batch = tf.constant(4, dtype=tf.int32)
    dummy_ebno = tf.constant(10.0, dtype=tf.float32)
    _ = model_train(dummy_batch, dummy_ebno, training=False)
    print("✅ Transformer built!\n")
    
    # ===== STEP 2: TRAIN WITH CORRECT SUM RATE =====
    print("[STEP 2] Training with Correct Sum Rate Calculation...")
    
    trainer = SumRateTrainer(model_train, learning_rate=5e-4)
    ...
    # Save
    model_train.save_weights("checkpoints/transformer_correct_trained.weights.h5")
    print("✅ Weights saved!\n")
    
    # ===== STEP 3: EVALUATE AGAINST RZF =====
    print("[STEP 3] Comparing with RZF...")
    
    model_rzf = MultiUserMIMO_Downlink(
        scenario="umi",
        perfect_csi=True,
        precoder_type="rzf",
        num_ut=4,
    ...
    # ===== STEP 4: VERIFY ACROSS SNR RANGE =====
    print("\n[STEP 4] Performance Across SNR Range...")
    
    ebno_range = [0, 5, 10, 15, 20]
    
    print(f"\n{'SNR (dB)':<10} {'Transformer':<15} {'RZF':<15} {'Gain (%)':<10}")
    print("-" * 50)
    
    for ebno in ebno_range:
        comp = evaluate_and_compare(model_train, model_rzf, ebno_db=ebno, num_batches=10)
        print(f"{ebno:<10} {comp['transformer']:<15.2f} {comp['rzf']:<15.2f} {comp['gain_pct']:<10.1f}")
    
    return training_history, comparison


if __name__ == "__main__":
    results = main()

def evaluate_precoder_with_rate(model, ebno_range, batch_size=128, num_batches=10):
    """
    Evaluate BER and sum rate for a precoder
    
    Returns:
        ber_dict: {'ber_total': array, 'ber_per_user': array}
        rate_dict: {'sum_rate_total': array, 'rate_per_user': array}
    """
    from evaluate_per_user import compute_ber_per_user
    ...
    for ebno_db in ebno_range:
        print(f"  Evaluating at Eb/N0 = {ebno_db:.1f} dB...", end='')
        ...
        for _ in range(num_batches):
            # Get bits
            b, b_hat = model(batch_size, ebno_tf, training=False)
            
            # Compute BER
            ber_total = compute_ber(b, b_hat)
            ber_per_user = compute_ber_per_user(b, b_hat)
            
            ber_batch_list.append(ber_total.numpy())
            ber_user_batch_list.append(ber_per_user.numpy())
        
        # Average over batches
        ber_total_list.append(np.mean(ber_batch_list))
        ber_per_user_list.append(np.mean(ber_user_batch_list, axis=0))
        
        print(f" BER = {ber_total_list[-1]:.2e}")
    
    ber_dict = {
        'ber_total': np.array(ber_total_list),
        'ber_per_user': np.array(ber_per_user_list).T  # [num_users, num_snr]
    }
    
    # Dummy rate for now (you can add sum rate calculation)
    rate_dict = {
        'sum_rate_total': np.zeros_like(ber_total_list),
        'rate_per_user': np.zeros((4, len(ebno_range)))
    }
    
    return ber_dict, rate_dict

def plot_constellation_debug(simulator, batch_size=64, ebno_db=10.0, save_path=None):
    """
    ✅ UPDATED: Works with both CDL and 3GPP channel models
    ...
    x_original = simulator._mapper(c)
    x_rg = simulator._rg_mapper(x_original)
    
    # ✅ Channel - works with both CDL and 3GPP models
    if hasattr(simulator, 'new_topology'):
        simulator.new_topology(batch_size)  # Set topology for 3GPP models
    
    # Use the channel application from the simulator
    if hasattr(simulator, '_ofdm_channel'):
        # New approach with OFDMChannel
        y, h_freq = simulator._ofdm_channel(x_rg, no)
    else:
        # Old approach with manual channel
        if hasattr(simulator, '_cdl'):
            cir = simulator._cdl(batch_size, simulator._rg.num_ofdm_symbols, 
                               1 / simulator._rg.ofdm_symbol_duration)
        else:
            cir = simulator._channel_model(batch_size, simulator._rg.num_ofdm_symbols,
                                          1 / simulator._rg.ofdm_symbol_duration)
    ...
    if simulator._perfect_csi:
        if hasattr(simulator, '_remove_nulled_subcarriers'):
            h_hat = simulator._remove_nulled_subcarriers(h_freq)
        else:
            h_hat = h_freq
        err_var = 0.0
    else:
    ...
    c_hat = simulator._encoder(b_hat)
    x_decoded = simulator._mapper(c_hat)
    ...
    stats_text = (
        ...
        f"Scenario: {getattr(simulator, '_scenario', 'CDL')}\n"
