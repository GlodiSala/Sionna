"""
figA_sumrate_csi_perfect.py — Figure A, UMa MASSIVE (M=64, K=8),
VARIANTE D=384,L=4 (réplique du chantier UMi du 12/08 sur canal UMa,
cf. RAPPORT_MASSIVE_UMA_GAP_CLOSING). Double panneau (gauche : CSI
parfait, droite : CSI imparfait pilote 20dB). RZF, WMMSE, SC, IB,
TA-RB (T=6), les 3 architectures neuronales à D=384 (au lieu de D=128
dans `uma_massive/`, référence intacte), même budget 170ep.
...
csi_imp = json.load(open(DATA_DIR + 'diag_csi_imperfect_massive_uma_capD384L4.json'))['pilot_results']['20.0']
...
fig.suptitle('Débit somme vs SNR — UMa MASSIVE (M=64, K=8), D=384', fontsize=14, fontweight='bold', y=1.02)
fig.tight_layout()
...
print('✅ Figure A (UMa MASSIVE, D=384) -> figA_sumrate_csi_perfect.{png,pdf,json}')
