...
1. Non-orthogonality levers that Sionna actually exposes (investigated by
   ...
   eval_system.py -- Sionna's own gen_single_sector_topology
...
2. WMMSE convergence ruled out as an explanation for any RZF/WMMSE gap
   ...
   documented 10-iteration default (precoders/classical.py:wmmse_precoder) --
...
hardcoded 72s in the active pipeline (eval_system.py,
precoders/rb_grouping.py, system.py, datasets.py/precoders_w.py/
precoders/base_intra_rb.py default args); left legacy/superseded scripts
...
same methodology as classical_reference.py, WMMSE already bisection-fixed):
...
with the same rigor as classical_reference.py (10 batches instead of
