"""
diag_massive_cluster_sweep.py — MASSIVE (M=64), même mécanisme de
proximité spatiale que STANDARD (R=20m, le rayon gagnant), sweep K
(4/8/16) pour trouver le meilleur compromis M>>K + gap observable.
"""
...
results = {}
for K in K_VALUES:
    batch = 16 if K <= 8 else 8
    print(f'\n{"="*75}\nMASSIVE M=64,K={K}  M/K={M/K:.1f}  R={R}m\n{"="*75}', flush=True)
    t0 = time.time()
    system = ClusteredSystem(M, K)
    system.cluster_radius_m = R
    system.new_topology(min(32, batch * 2))
    h_freq, _ = system.channel_and_no(tf.constant(min(32, batch * 2), tf.int32), tf.constant(10.0, tf.float32))
    ...
    results[str(K)] = {'M': M, 'K': K, 'M_over_K': M / K, 'corr': corr,
...
print(f'\n\n=== RÉSUMÉ MASSIVE, cluster R=20m ===')
print(f'{"K":>4} {"M/K":>6} {"rho@95sc":>9} {"max_gap%":>9} {"pts>0.3%":>9}')
for K, r in results.items():
    print(f'{K:>4} {r["M_over_K"]:>6.1f} {r["corr"][95]:>9.3f} {r["max_gap_pct"]:>9.2f} {r["n_points_with_gap_gt_0.3pct"]:>9d}')

with open('results/diag_massive_cluster_sweep.json', 'w') as f:
    json.dump(results, f, indent=2)
print('\nSauvé -> results/diag_massive_cluster_sweep.json')
