"""
figA_sumrate_csi_perfect.py — Figure A, UMi MASSIVE (M=64, K=8),
VARIANTE D=384,L=4 (investigation "plafond 73-80% RZF" du 12/08, cf.
SESSION_LOG_20260808.md). Double panneau (gauche : CSI parfait, droite
: CSI imparfait pilote 20dB). RZF, WMMSE, SC, IB, TA-RB (T=6), les 3
architectures neuronales à D=384 (au lieu de D=128 dans la figure de
production `umi_massive/`), même budget 170ep (10 warmup + 160
finetune).

Figure de COMPARAISON, dossier séparé de `umi_massive/` (référence
D=128, INTACTE) -- ne pas confondre les deux versions.
...
csi_imp = json.load(open(DATA_DIR + 'diag_csi_imperfect_massive_capD384L4.json'))['pilot_results']['20.0']
...
print('✅ Figure A (UMi MASSIVE, D=384) -> figA_sumrate_csi_perfect.{png,pdf,json}')
