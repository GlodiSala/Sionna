"""
tsweep_seedfix_v2.py -- tab:ablation_T (T=1,2,3,4,6,12), CSI parfait ET
imparfait (pilote 20dB), meme methodologie que classical_comparison_
seedfix_v2.py : seed reel (sionna_config.seed), canal PARTAGE par batch
entre RZF-SC, RZF-RB12, WMMSE et les 6 T, double metrique (Sionna
LMMSEPostEqualizationSINR vs SINR manuel direct).

Corrige par rapport aux scripts originaux (figC_sumrate_snr_T.py,
diag_csi_imperfect_tsweep.py) :
  - bug 1 (metrique Sionna 2x-interference) : ajoute le SINR manuel en
    parallele.
  - bug 2 (canal non partage) : un seul h_true tire par batch, reutilise
    pour RZF-SC/RZF-RB12/WMMSE ET les 6 T (au lieu d'un tirage independant
    par T et par rapport a RZF/WMMSE).
  - bugs 3 (checkpoint) et 4 (echantillonnage) : verifies sains pour ce
    T-sweep (tous les config.json montrent 43-80 epoques, evals internes
    croissants -- pas de snapshot sous-entraine ; diag_csi_imperfect_
    tsweep.py utilisait deja 20x32=640 echantillons, largement suffisant).
...
outputs = {
    'tsweep_seedfix_perfect_sionna': {'metric': 'sionna', 'csi': 'perfect', 'table': build_table(results_perfect_sionna), 'metadata': base_meta},
    'tsweep_seedfix_perfect_manual': {'metric': 'manual', 'csi': 'perfect', 'table': build_table(results_perfect_manual), 'metadata': base_meta},
    'tsweep_seedfix_pilot20_sionna': {'metric': 'sionna', 'csi': 'pilot20dB', 'table': build_table(results_pilot_sionna), 'metadata': base_meta},
    'tsweep_seedfix_pilot20_manual': {'metric': 'manual', 'csi': 'pilot20dB', 'table': build_table(results_pilot_manual), 'metadata': base_meta},
}
