"""
diag_csi_imperfect_massive_capD384L4.py — CSI imparfait pour les 3
architectures signed_attn réentraînées à D=384,L=4 (budget complet
170ep) sur MASSIVE_TRUE (M=64,K=8), suite à l'investigation "plafond
73-80% RZF" du 12/08 (cf. SESSION_LOG). Réplique EXACTE de la
méthodologie de `diag_csi_imperfect_massive_extbudget_sweep.py`
(production D=128) -- seul embed_dim et les chemins de checkpoints
changent. Sauvé dans un fichier SÉPARÉ (pas d'écrasement de la
référence D=128 extbudget).

Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 diag_csi_imperfect_massive_capD384L4.py
"""
...
if __name__ == '__main__':
    ...
    for name in RESULT_JSONS:
        ...
        print(f'✅ {name} (D={D}) chargé depuis {ckpt}', flush=True)
    ...
    for data_snr in DATA_SNRS_DB:
        ...
        with open('results/diag_csi_imperfect_massive_capD384L4.json', 'w') as f:
            json.dump({'pilot_results': results, 'perfect': perfect_cache,
                       'M': M, 'K': K, 'D': D, 'L': L}, f, indent=2)

    print('\n✅ Sauvé -> results/diag_csi_imperfect_massive_capD384L4.json')
