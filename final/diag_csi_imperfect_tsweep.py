"""
diag_csi_imperfect_tsweep.py — Figure B double panneau (demande
utilisateur) : étend le sweep CSI imparfait (pilote 20dB, méthodologie
diag_csi_imperfect_signed_attn_sweep.py) à TOUS les T du T-sweep, pas
seulement T=4 -- nécessaire pour tracer le Pareto débit/énergie sous
CSI imparfait avec la même courbe "TA-RB par T" que sous CSI parfait.

Un T par invocation (parallélisable sur plusieurs GPU, comme le
T-sweep d'entraînement). Résultats fusionnés dans un JSON séparé par T
(results/diag_csi_imperfect_tsweep_T{T}.json) -- pas de risque d'écraser
diag_csi_imperfect_signed_attn_sweep.json (qui ne contient que T=4,
utilisé par la Figure A).

Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 diag_csi_imperfect_tsweep.py --T 6
"""
...
BATCH = 32
SEED = 2026
T_RESULT_JSON = f'results/diag_tarb_residual_signed_attn_T{T}_train.json'
OUT_JSON = f'results/diag_csi_imperfect_tsweep_T{T}.json'
...
if __name__ == '__main__':
    ...
    print(f'✅ TA-RB T={T} chargé depuis {ckpt}', flush=True)
    ...
    for data_snr in DATA_SNRS_DB:
        ...
        print(f'  T={T} data_snr={data_snr}dB perfect={perf:6.2f} pilot20dB={pil:6.2f} ({pct:5.1f}%)', flush=True)

    with open(OUT_JSON, 'w') as f:
        json.dump({'T': T, 'results': results}, f, indent=2)
    print(f'\n✅ Sauvé -> {OUT_JSON}')
