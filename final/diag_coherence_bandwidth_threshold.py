"""
diag_coherence_bandwidth_threshold.py — Investigation demandée (François,
...
(UMi~0,89 / UMa~0,65, cf. diag_coherence_annexe_b.py, résultats
...
Méthodologie de mesure identique à diag_coherence_annexe_b.py :
  rho(lag) = |mean(H[...,:-lag] * conj(H[...,lag:]))| / mean(|H|^2)
moyennée sur tirages (NUM_DRAWS), batch, utilisateurs, antennes.

Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 diag_coherence_bandwidth_threshold.py
"""
import os, sys, json
import numpy as np
...
SEED = 2026                        # reproductibilité -- diag_coherence_annexe_b.py n'en fixait pas
...
LAGS = list(range(1, 96))          # DENSE : chaque sous-porteuse (vs sous-ensemble épars de diag_coherence_annexe_b.py)
NUM_DRAWS = 100                    # >> diag_coherence_annexe_b.py (20) -- réduit le bruit Monte-Carlo observé
