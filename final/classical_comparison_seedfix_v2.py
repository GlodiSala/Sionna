"""
classical_comparison_seedfix_v2.py -- combine deux corrections en un seul run :
...
Ne modifie ni classical_comparison.py ni classical_comparison_seedfix.py.

Usage: python3 classical_comparison_seedfix_v2.py [--num_batches N]
"""
...
def sionna_rate(system, h_freq, g, no):
    ...
    (wmmse_convergence_check.py:174-181)."""
...
def manual_sinr_rate(stream_management, h_freq, g, no):
    ...
    -- meme formule que precoders_w.py:103-110, sans lmmse_matrix ni blanchiment."""
    g = normalize_g(g)
...
def main():
    ...
    out_manual = {
        'metric': 'manual_sinr (SINR_k = |h_k^H w_k|^2 / (sum_j!=k |h_k^H w_j|^2 + no), precoders_w.py:103-110)',
        'table': manual_table,
        'metadata': base_meta,
